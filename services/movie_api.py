...
def get_movies_genre_list():
    ...
    return [{"name": genre.get("name", "不明"), "id": genre.get("id")} for genre in data.get("genres", [])]
...
def get_subscription_providers():
    ...
    data = make_request(url, params)
    return [{"name": provider.get("provider_name", "不明"), "id": provider.get("provider_id")} for provider in data.get("results", [])]
...
def search_movies_by_filters(genre=None, year=None, subscriptions=None):
    ...
    for provider in subscriptions:
        provider_params = {**params, "with_watch_providers": provider, "watch_region": "JP"}
...
def get_movie_details(movie_id):
    ...
    still_urls = [f"https://image.tmdb.org/t/p/original{img['file_path']}" for img in images_data.get('backdrops', [])]
    ...
    return {
        ...
        "genres": [genre.get("name", "不明") for genre in movie_details.get("genres", [])],
